...
def convert_to_shift(model, shift_depth, shift_type, convert_all_linear=True, convert_weights=True, freeze_sign = False, use_kernel=False, use_cuda=True,
    rounding='deterministic', weight_bits=5, sign_threshold=None, quant_bits=16):
    conversion_count = 0
            ...
            if shift_type == 'Q':
                ...
                if use_cuda==True and use_kernel == True:
                    ...
                    shift_linear = deepshift.modules.LinearShift(module.in_features, module.out_features, module.bias is not None, freeze_sign = freeze_sign, use_kernel=use_kernel,
                        use_cuda=use_cuda, rounding=rounding, weight_bits=weight_bits, threshold=sign_threshold)
            ...
            if shift_type == 'Q':
                ...
                if use_cuda==True and use_kernel == True:
                    ...
                    shift_conv2d = deepshift.modules.Conv2dShift(module.in_channels, module.out_channels, module.kernel_size, module.stride,
                                                ...
                                                rounding=rounding, weight_bits=weight_bits, threshold=sign_threshold, quant_bits=quant_bits)
                    if convert_weights == True:
                    ...
                    #print(conv2d.kernel_size)

            if conv2d.kernel_size==(1,1):
