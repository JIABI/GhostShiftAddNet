...
def summary(model, input_size, batch_size=64, device=torch.device('cuda:0'), dtypes=None):
...
def summary_string(model, input_size, batch_size=-1, device="cuda", dtypes=None):
    model, conversion_count = convert_to_shift(model, 1, convert_weights=True, shift_type ='PS',
    ...
    def register_hook(module):
        ...
        def hook(module, input, output):
            ...
            if hasattr(module, "shift") and hasattr(module.shift, "size"):
                assert (hasattr(module, "shift"))
                assert (hasattr(module.shift, "size"))
            ...
            if hasattr(module, "adder") and hasattr(module.adder, "size"):
                assert (hasattr(module, "adder"))
                assert (hasattr(module.adder, "size"))

                add_params = torch.prod(torch.LongTensor(list(module.adder.size())))
                #params += add_params

                #summary[m_key]["trainable"] = module.adder.requires_grad
            if hasattr(module, "bias") and hasattr(module.bias, "size"):
                bias_params = torch.prod(torch.LongTensor(list(module.bias.size())))
                params += 0
                params_bits += bias_params * 32
            if hasattr(module, "running_mean") and hasattr(module.running_mean, "size") and hasattr(module,
            ...
            if hasattr(module, "running_var") and hasattr(module.running_var, "size") and hasattr(module,
                                                                                                  "track_running_stats") and module.track_running_stats:
            ...
            summary[m_key]["nb_shift_params"] = shift_params
            summary[m_key]["nb_adder_params"] = add_params
...
#        dtype = torch.FloatTensor
    ...
    for layer in summary:
        ...
        total_shift_params += summary[layer]["nb_shift_params"]
        total_add_params += summary[layer]["nb_adder_params"]
    ...
    summary_str += "Total shift params: {0:,}".format(total_shift_params) + "\n"
    summary_str += "Total adder params: {0:,}".format(total_add_params) + "\n"
