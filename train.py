...
parser.add_argument('--resume', default='./ShiftAddNet_ckpt/shiftaddnet/resnet20-cifar10-FP32.pth.tar', type=str, metavar='PATH', help='path to latest checkpoint')
parser.add_argument('--seed', type=int, default=1, metavar='S', help='random seed')
parser.add_argument('--save', default='./temp', type=str, metavar='PATH', help='path to save prune model')
parser.add_argument('--arch', default='resnet20_shiftadd', type=str, help='architecture to use')
...
for name, m in model.named_modules():
        if isinstance(m, deepshift.modules.Conv2dShift):
            m.shift_grad = torch.zeros_like(m.shift.data)
            m.shift_mean_grad = torch.zeros_like(m.shift.data)
            m.sign_grad = torch.zeros_like(m.shift.data)
            m.sign_mean_grad = torch.zeros_like(m.shift.data)
            m.shift_sum = torch.zeros_like(m.shift.data)
            m.shift_mask  = torch.zeros_like(m.shift.data.min().float())
        if isinstance(m, Adder2D):
            m.adder_grad = torch.zeros_like(m.adder.data)
            m.adder_mean_grad = torch.zeros_like(m.adder.data)
            m.mask = torch.zeros_like(m.adder.data.min().float())
            m.adder_sum = torch.zeros_like(m.adder.data)
...
#    batch_time = time.time()
    ...
    # with torch.no_grad():
        ...
        # update shift adder grads:
        #for name, m in model.named_modules():
        #    if isinstance(m, deepshift.modules.Conv2dShift):
        #        m.shift.grad.data = m.shift.grad.data - m.shift_grad + m.shift_mean_grad
        #    if isinstance(m, Adder2D):
        #        m.adder.grad.data = m.adder.grad.data - m.adder_grad + m.adder_mean_grad
...
if __name__ == '__main__':
  for epoch in range(args.start_epoch, args.epochs):
    ...
    np.savetxt(os.path.join(args.save, 'record_sparse_or.txt'), history_score, fmt='%10.5f', delimiter=',')
 #   np.savetxt(os.path.join(args.save, 'record1.txt'), history_score1, fmt='%10.5f', delimiter=',')
 #   save_checkpoint({
 #       'epoch': epoch + 1,
