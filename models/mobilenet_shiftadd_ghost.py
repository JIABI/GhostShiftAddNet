...
__all__ = ['mobilenet_shiftadd_ghost']


def conv3x3(in_planes, out_planes, kernel_size, stride, padding, groups, bias=False, quantize=False, weight_bits=8, quantize_v='sbm'):
    " 3x3 convolution with padding "
    shift = nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=padding, bias=bias, groups = groups)
...
class GhostModule(nn.Module):
    def __init__(self, inp, oup, kernel_size=1, ratio=2, dw_size=3, stride=1, relu=True):
        ...
        self.primary_conv = nn.Sequential(
            conv3x3(inp, init_channels, kernel_size, stride, kernel_size // 2, groups=1, bias=False),
...
#        )
        self.cheap_operation = nn.Sequential(
            conv5x5(init_channels, new_channels, dw_size, stride, dw_size // 2, groups=init_channels, bias=False),
            nn.BatchNorm2d(new_channels),
            nn.ReLU(inplace=True) if relu else nn.Sequential(),
        )
...
class SqueezeExcite(nn.Module):
    def __init__(self, in_chs, se_ratio=0.25, reduced_base_chs=None,
        ...
        self.conv_reduce = conv5x5(in_chs, reduced_chs, kernel_size=1, stride=1, padding=0, groups=1, bias=False)
        #self.conv_reduce = conv5x5(in_chs, reduced_chs, kernel_size=1, stride=1, padding=0, groups=1, bias=False)
        self.act1 = act_layer(inplace=True)
        self.conv_expand = nn.Conv2d(reduced_chs, in_chs, kernel_size=1, stride=1, padding=0, groups=1, bias=False)
        #self.conv_expand = conv5x5(reduced_chs, in_chs, kernel_size=1, stride=1, padding=0, groups=1, bias=False)
...
class ConvBnAct(nn.Module):
    def __init__(self, in_chs, out_chs, kernel_size,
        ...
        self.conv = conv5x5(in_chs, out_chs, kernel_size, stride, kernel_size // 2, groups=1, bias=False)
...
class GhostBottleneck(nn.Module):
    ...
    def __init__(self, in_chs, mid_chs, out_chs, dw_kernel_size=3,
                 stride=1, act_layer=nn.ReLU, se_ratio=0.25, downsample=None, quantize=False, weight_bits=8, quantize_v='sbm'):
        ...
        if self.stride > 1:
           self.conv_dw = conv3x3(mid_chs, mid_chs, dw_kernel_size, stride=stride, padding=(dw_kernel_size - 1) // 2, groups=mid_chs, bias=False, quantize=quantize, weight_bits=weight_bits, quantize_v=quantize_v)
        ...
        if (in_chs == out_chs and self.stride == 1):
            ...
            self.shortcut = nn.Sequential(
                conv3x3(in_chs, in_chs, dw_kernel_size, stride=stride,
...
class ResNet(nn.Module):
    def __init__(self, cfgs, num_classes=10, kernel_size=3, width=1.0, dropout=0.1, quantize=False, weight_bits=8, quantize_v='sbm'):
        ...
        self.conv_stem = conv3x3(3, planes, 3, 1, 1, groups= 1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
...
#    return ResNet(cfgs, num_classes=num_classes, **kwargs)
