...
__all__ = ['shufflenet_shiftadd_ghost_cpu']
...
def conv5x5(in_planes, out_planes, kernel_size, stride, padding, bias=False, quantize=False, weight_bits=8, quantize_v='sbm'):
    " 3x3 convolution with padding "
    shift = nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, padding=padding, stride=stride, bias=bias)
...
class GhostModule(nn.Module):
    def __init__(self, inp, oup,  ratio, kernel_size=1, dw_size=3, stride=1, relu=True, padding=0, bias=False):
...
#        new_channels = math.ceil(init_channels * (1-self.ratio))
 ...
 #       new_channels = init_channels*(ratio-1)
        ...
        self.primary_conv = nn.Sequential(
            conv_add(inp, init_channels, kernel_size, stride, kernel_size//2,  bias=False),
            nn.BatchNorm2d(init_channels),
            nn.ReLU(inplace=True) if relu else nn.Sequential(),
        )

        self.cheap_operation = nn.Sequential(
            conv5x5(init_channels, new_channels, dw_size, 1, dw_size//2, bias=False),
...
class GhostModule_conv(nn.Module):
    def __init__(self, inp, oup,  ratio, kernel_size=3, dw_size=3, stride=1, relu=True, padding=0, bias=False):
        super(GhostModule_conv, self).__init__()
        self.oup = oup
        self.ratio = ratio
...
class InvertedResidual(nn.Module):
    def __init__(self, inp : int, oup : int, stride: int, ratio: float):
 ...
 #           raise ValueError('illegal stride value')
        ...
        if self.stride > 1:
            ...
            #    nn.Conv2d(inp, inp, stride=self.stride, kernel_size=3, padding=1, bias=False, groups=inp),
            #    nn.BatchNorm2d(inp),
...
#                nn.BatchNorm2d(branch_features),
        ...
        self.branch2 = nn.Sequential(
            ...
            nn.BatchNorm2d(branch_features),
            nn.ReLU(inplace=True),
        #    GhostModule(branch_features, branch_features, ratio=self.ratio, kernel_size=3, stride=1),
            #nn.Conv2d(branch_features, branch_features, kernel_size=1, stride=1, padding=0, bias=False),
            # GhostModule(inp if (self.stride > 1) else branch_features,
            #          branch_features, kernel_size=1, stride=1, padding=0, bias=False, ratio=self.ratio, relu=True),
            GhostModule(branch_features, branch_features, kernel_size=3, stride=self.stride, padding=1, bias=False, ratio=self.ratio, relu=True),
            #nn.Conv2d(branch_features, branch_features, kernel_size=3, stride=self.stride, padding=1, groups=branch_features, bias=False),
            nn.BatchNorm2d(branch_features),
            nn.Conv2d(branch_features, branch_features, kernel_size=1, stride=1, padding=0, bias=False),
            #GhostModule(branch_features, branch_features, kernel_size=1, stride=1, padding=0, bias=False, ratio=self.ratio, relu=True),
            nn.BatchNorm2d(branch_features),
            nn.ReLU(inplace=True),
...
class ShuffleNetV2(nn.Module):
 ...
 #           nn.ReLU(inplace=True),
 #           conv3x3(input_channels, output_channels, kernel_size=3, stride=1, padding=0, bias=False,),
 #           nn.BatchNorm2d(output_channels),
 #           nn.ReLU(inplace=True),

        self.conv_shift = nn.Sequential(
            nn.Conv2d(input_channels, input_channels, 1, 1, 0, bias=False),
        #    GhostModule(input_channels, input_channels, kernel_size=1, stride=1, padding=0, bias=False, ratio=self.ratio,relu=True),
        #    nn.BatchNorm2d(input_channels),
        #    nn.ReLU(inplace=True),
        #    nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
        #    conv5x5(input_channels, output_channels, 3, 1, 1, groups=1, bias=False),
        #    nn.BatchNorm2d(output_channels),

        #    GhostModule(input_channels, input_channels, kernel_size=1, stride=1, padding=0, bias=False, ratio=2, relu=True),
            nn.BatchNorm2d(input_channels),
            nn.ReLU(inplace=True),
 #           nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
        ...
        self.conv1x1 = nn.Sequential(
         ...
         #   GhostModule(output_channels, output_channels, kernel_size=1, stride=1, padding=0, bias=False, ratio=self.ratio,relu=True),
         #   nn.BatchNorm2d(output_channels),
         #   GhostModule(input_channels, output_channels, kernel_size=1, stride=1, padding=0, bias=False, ratio=self.ratio, relu=True),
            nn.BatchNorm2d(output_channels),
            nn.ReLU(inplace=True),
        )
        self.conv1 = nn.Sequential(
            nn.Conv2d(input_channels, output_channels, kernel_size=3, stride=2, padding=1, groups=input_channels, bias=False),
            nn.BatchNorm2d(output_channels),
            nn.ReLU(inplace=True),
        )
        ...
        for name, repeats, output_channels in zip(
            ...
            if name == 'stage2':
                seq = [inverted_residual(inp=input_channels, oup=output_channels, stride=2, ratio=0.9)]
            elif name == 'stage3':
                seq = [inverted_residual(inp=input_channels, oup=output_channels, stride=2, ratio=0.5)]
            elif name == 'stage4':
                seq = [inverted_residual(inp=input_channels, oup=output_channels, stride=2, ratio=0.1)]

            for i in range(repeats-1):
                if name=='stage2':
                   seq.append(InvertedResidual(output_channels, output_channels, 1, ratio=0.9))
                elif name=='stage3':
                   seq.append(InvertedResidual(output_channels, output_channels, 1, ratio=0.5))
                elif name=='stage4':
                   seq.append(InvertedResidual(output_channels, output_channels, 1, ratio=0.1))
        ...
        self.conv5 = nn.Sequential(
            #GhostModule(input_channels, output_channels, kernel_size=1, stride=1, padding=0, bias=False,ratio=4, relu=True),
...
def ghostnet(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ShuffleNetV2:
    ...
    """
    return _shufflenetv2('shufflenetv2_x1.0', pretrained=True, progress=False, kernel_size=3,
