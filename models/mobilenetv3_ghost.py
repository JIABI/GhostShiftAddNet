...
def conv_add(in_planes, out_planes, kernel_size, stride, padding, bias=False, quantize=False, weight_bits=8, quantize_v='sbm'):
    " 3x3 convolution with padding "
    add = adder.Adder2D(in_planes, out_planes, kernel_size=kernel_size, stride=stride, groups=1, padding=padding, bias=bias, quantize=quantize, weight_bits=weight_bits, quantize_v=quantize_v)
    #add = adder2d(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=padding, bias=bias)
...
class GhostModule(nn.Module):
    def __init__(self, inp, oup, kernel_size, ratio=2, stride=1, relu=True, dilation=1,):
...
class GhostModule_DW(nn.Module):
    def __init__(self, inp, oup, kernel_size=1, ratio=2, dw_size=5, groups=1, stride=1, relu=True, dilation=1,):
        super(GhostModule_DW, self).__init__()
        self.oup = oup
        init_channels = math.ceil(oup / ratio)
        new_channels = init_channels * (ratio - 1)
        padding = (kernel_size - 1) // 2 * dilation
        ...
        self.primary_conv = nn.Sequential(
            nn.Conv2d(inp, init_channels, kernel_size, 1, kernel_size // 2, groups=1, bias=False),
            nn.BatchNorm2d(init_channels),
...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, cnf: InvertedResidualConfig, norm_layer: Callable[..., nn.Module],
        ...
        if cnf.expanded_channels != cnf.input_channels:
            layers.append(GhostModule(cnf.input_channels, cnf.expanded_channels, kernel_size=cnf.kernel))
            #layers.append(nn.BatchNorm2d(cnf.expanded_channels))
            layers.append(nn.ReLU())

            #layers.append(ConvBNActivation(cnf.input_channels, cnf.expanded_channels, kernel_size=1,
            #                               norm_layer=norm_layer, activation_layer=activation_layer))
            #layers.append(
            #    conv_1x1(cnf.input_channels, cnf.expanded_channels, kernel_size=1, stride=1, padding=1 // 2, bias=False))
        ...
        layers.append(nn.MaxPool2d(kernel_size=3, stride=stride, padding=3//2))
        layers.append(nn.BatchNorm2d(cnf.expanded_channels))
        #layers.append(nn.ReLU())
        ...
        layers.append(GhostModule(cnf.expanded_channels, cnf.out_channels, kernel_size=1))
        #layers.append(conv_1x1(cnf.expanded_channels, cnf.out_channels, kernel_size=1, stride=1, padding=1 // 2, bias=False))
        #layers.append(nn.BatchNorm2d(cnf.out_channels))
...
def _mobilenet_v3_conf(arch: str, params: Dict[str, Any]):
    ...
    width_mult = params.pop('_width_mult', 0.25)
